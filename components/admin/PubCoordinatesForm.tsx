...
export function PubCoordinatesForm({ 
  ...
  const updateCoordinate = (pubId: string, field: 'latitude' | 'longitude', value: string) => {
    setCoordinates(prev => 
      prev.map(coord => 
        coord.id === pubId 
          ? { ...coord, [field]: value }
  ...
  const saveCoordinates = async () => {
    ...
    try {
      ...
      const invalidCoords = coordinates.filter(coord => {
        ...
        return !validateCoordinate(lat, 'latitude') || !validateCoordinate(lng, 'longitude');
      });

      if (invalidCoords.length > 0) {
        alert(`Invalid coordinates found for: ${invalidCoords.map(c => c.name).join(', ')}`);
        setSaving(false);
        return;
      }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Enter GPS coordinates for each pub. Get these from Google Maps by right-clicking and selecting "What's here?"
          </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateCoordinate(coord.id, 'latitude', e.target.value)}
                    className={`bg-slate-900/50 border-slate-600 text-white text-sm ${
                      !latValid ? 'border-red-500' : ''
                    }`}
                  />
                  {!latValid && (
                    <p className="text-xs text-red-400 mt-1">Invalid latitude</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateCoordinate(coord.id, 'longitude', e.target.value)}
                    className={`bg-slate-900/50 border-slate-600 text-white text-sm ${
                      !lngValid ? 'border-red-500' : ''
                    }`}
                  />
                  {!lngValid && (
                    <p className="text-xs text-red-400 mt-1">Invalid longitude</p>
                  )}
                </div>
              </div>
            </div>
      ...
      {validCount > 0 && (
        <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-3">
          <h4 className="text-blue-300 font-medium text-sm mb-2">Instructions for Google Maps:</h4>
          <ol className="text-xs text-blue-200 space-y-1">
            <li>1. Go to Google Maps and search for "[Pub Name] Fremantle"</li>
